...
- **`risk_engine.py` contains the active risk logic** — equipment/outage scoring, impact analysis, recommendations, and the rule-based copilot response generation live there.
- **`src/backend/data/equipment.csv` is a committed runtime dataset** — the backend loads it on startup; keep this file tracked.
- The FastAPI `app` object is defined in `src/backend/main.py`, importable as `src.backend.main:app`.
...
4. `submission.yaml` `tech_stack` arrays are populated metadata fields; CI does not currently enforce their contents
